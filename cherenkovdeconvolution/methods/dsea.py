...
def deconvolve(X_data, X_train, y_train, classifier,
    ...
    fixweighting : str, optional
        Whether or not the weight update fix is applied, which is proposed in my Master's
        thesis and the corresponding paper.
        NOTE: CHANGED!
        Can be 'always', 'never', or 'iter' (new).
        'iter' means that the fix is not applied during initialisation.
    ...
    w_train = _dsea_weights(y_train, f / f_train if (fixweighting == 'always') else f) # instance weights
    if inspect is not None:
        inspect(_recode_result(f, recode_dict), None, 0, np.nan, np.nan)
    ...
    for k in range(1, K+1):
        ...
        if inspect is not None:
            inspect(_recode_result(f, recode_dict), proba, k, alphak, chi2s)
        ...
        if k < K:
            if smoothing is not None:
                f = smoothing(f)
            w_train = _dsea_weights(y_train, f / f_train if (fixweighting in ['always', 'iter']) else f)
        # = = = = = = = = = = = = = = = = = = = = = = = = = = =
